...
describe('ElasticsearchService', () => {
  ...
  beforeEach(async () => {
    ...
    mockNestElasticsearchService.indices.exists.mockResolvedValue({
      body: false,
    })
    mockNestElasticsearchService.indices.create.mockResolvedValue({
    ...
    mockNestElasticsearchService.search.mockResolvedValue({
      body: {
        hits: {
          hits: [{ _source: { id: '1', customerName: 'Test Customer' } }],
        },
      },
    })
    ...
    nestElasticsearchService = module.get<NestElasticsearchService>(
      NestElasticsearchService,
    )
  ...
  describe('indexOrder', () => {
    ...
    it('should handle errors gracefully', async () => {
      ...
      mockNestElasticsearchService.index.mockRejectedValueOnce(
  ...
  describe('removeOrderIndex', () => {
    it('should attempt to remove an order from Elasticsearch', async () => {
      ...
      const removeOrderSpy = jest.spyOn(service, 'removeOrderFromIndex')

      await service.removeOrderFromIndex('1')
  ...
  describe('search', () => {
    ...
    it('should handle search errors gracefully', async () => {
      ...
      mockNestElasticsearchService.search.mockRejectedValueOnce(
