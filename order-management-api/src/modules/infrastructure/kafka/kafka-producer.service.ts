  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  constructor(private readonly configService: ConfigService) {
    const brokers = this.configService
      ...
      .split(',')
    this.maxConnectionAttempts = this.configService.get<number>(
      'KAFKA_RETRY_MAX',
      5,
    )
  ...
  async onModuleInit() {
    this.connect().catch((err) => {
      this.logger.warn(
        `Inicialização não bloqueada apesar do erro de conexão com Kafka: ${err.message}`,
      )
    })
  }
  ...
  async onApplicationShutdown() {
    if (this.isConnected) {
      try {
        ...
        this.logger.error(
          `Erro ao desconectar o produtor Kafka: ${error.message}`,
        )
      }
    }
  }
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Tentando conectar ao Kafka (tentativa ${this.connectionAttempts}/${this.maxConnectionAttempts})...`,
      )

      const connectionPromise = this.producer.connect()
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error('Timeout ao conectar ao Kafka')),
  ...
  private async produceInternal(
    ...
    if (!this.isConnected) {
      this.logger.warn(
        `Mensagem para o tópico ${topic} descartada porque o Kafka continua desconectado.`,
      )
      return
    }
    ...
    try {
      ...
      this.logger.error(
        `Erro ao enviar mensagem para o tópico ${topic}: ${error.message}`,
      )
      this.logger.warn(
        'A mensagem foi descartada, mas a aplicação continuará funcionando.',
      )
